...
@section('header')
    <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs font-medium text-slate-500 mt-1">Pantau seluruh riwayat transaksi pelanggan dan rekap pendapatan berdasarkan pilihan periode.</p>
        </div>
        ...
        <form method="GET" action="{{ route('merchant.orders.index') }}" class="flex items-center gap-2 bg-slate-900 p-2 rounded-2xl shadow-lg border border-slate-800">
            ...
            <div class="relative">
                <select name="filter_type" id="filterTypeSelect" onchange="switchFilterMode(this.value)"
                    class="bg-slate-800 text-amber-400 text-xs font-bold rounded-xl px-3 py-2 border border-slate-700/80 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer transition">
            ...
            <!-- Input Per Hari -->
            <div id="inputDayWrapper" class="{{ $filterType === 'day' ? 'block' : 'hidden' }}">
                <input type="date" name="date" value="{{ $selectedDate }}" 
                    class="bg-slate-800 text-white text-xs font-bold rounded-xl px-3 py-2 border border-slate-700/80 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer shadow-inner">
            </div>
            ...
            <div id="inputYearWrapper" class="{{ $filterType === 'year' ? 'block' : 'hidden' }}">
                <select name="year" class="bg-slate-800 text-white text-xs font-bold rounded-xl px-3 py-2 border border-slate-700/80 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer">
            ...
            <!-- Tombol Submit (Panah Dihapus) -->
            <button type="submit" class="bg-amber-500 hover:bg-amber-400 text-slate-950 px-4 py-2 rounded-xl text-xs font-black transition shadow-md shadow-amber-500/20 active:scale-95 cursor-pointer">
                Terapkan
            </button>
        </form>
    </div>
...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <!-- STATISTIK RINGKASAN (TOTAL PESANAN & PENDAPATAN) -->
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">Semua riwayat pesanan periode {{ $labelPeriode }}</p>
                </div>
            </div>
                            ...
                            <tr class="hover:bg-slate-50/60 transition">
                                ...
                                <td class="p-4 pr-6 text-center">
                                    <a href="{{ route('merchant.orders.receipt', $order->id) }}" target="_blank"
                                        class="inline-flex items-center justify-center w-9 h-9 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition shadow-sm active:scale-95"
                                        title="Cetak Struk">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-12 text-center">
                                    ...
                                    <p class="font-bold text-slate-700 text-sm">Tidak ada riwayat transaksi</p>
                                    <p class="text-xs text-slate-400 mt-1">Tidak ditemukan pesanan pada periode {{ $labelPeriode }}.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <!-- JavaScript Switch Mode Input Filter -->
    <script>
